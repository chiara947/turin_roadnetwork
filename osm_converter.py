...
for l_key, l_value in links_dict.items():
    ...
    links_dict[l_key]['length'] = [haversine.haversine(nodes_dict[x][0], nodes_dict[x][1], nodes_dict[y][0], nodes_dict[y][1]) for (x,y) in zip(l_n, l_n[1:])]
...
#with open('data_repo/tagged_alloneway_nodes.json', 'w') as nodes_in_links_outfile:
